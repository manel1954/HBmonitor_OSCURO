...
ORANGE      = 'ff8000'
WHITE       = 'ffffff'
WHITE2      = '262B31' # color del TS2 en reposo
GRIS        = '1D2024' # background Conected
ROJO        = 'F00000' # color Conected
YELLOW      = 'fffccd'
    ...
    for _bridge, _bridge_data in list(_bridges.items()):
        ...
        for system in _bridges[_bridge]:
            ...
            if system['ACTIVE'] == True:
                ...
                _stats_table[_bridge][system['SYSTEM']]['BGCOLOR'] = GRIS
            elif system['ACTIVE'] == False:
                _stats_table[_bridge][system['SYSTEM']]['ACTIVE'] = 'Disconnected'
                _stats_table[_bridge][system['SYSTEM']]['COLOR'] = ROJO
                _stats_table[_bridge][system['SYSTEM']]['BGCOLOR'] = GRIS
            ...
            if p[1] == 'END':
                ...
                if LASTHEARD_INC:
                   if int(float(p[9]))> 2: 
                      ...
                      f.write("<br><fieldset style=\"border-radius: 0px; background-color:#262B31; text-align: center; margin-left:15px;margin-right:15px;font-size:14px;\">\n")
                      ...
                      f.write("<TR style=\" height: 50px;font: 10pt arial, sans-serif; color:#979DA2;background-color:#262B31\"><TH>Date</TH><TH>Time</TH><TH>Callsign (DMR-Id)</TH><TH>Name</TH><TH>TG#</TH><TH>TG Name</TH><TH>TX (s)</TH><TH>Slot</TH><TH>Source ID</TH><TH>System</TH></TR>\n")
                      with open(LOG_PATH+"lastheard.log", "r") as textfile:
                          for row in islice(reversed(list(csv.reader(textfile))),100):
                            ...
                            if row[10] not in my_list:
                               if len(row) < 13:
                                   hline="<TR style=\"background-color:#182224; height:30px; color:#fff;\"><TD>"+row[0][:10]+"</TD><TD>"+row[0][11:16]+"</TD><TD><font color=#fff><b><a target=\"_blank\" href=https://qrz.com/db/"+row[11]+">"+row[11]+"</a></b></font><span style=\"font: 7pt arial,sans-serif\"> ("+row[10]+")</span></TD><TD><font color=#002d62><b></b></font></TD><TD><font color=#b5651d><b>"+row[8][2:]+"</b></font></TD><TD><font color=green><b>"+row[9]+"</b></font></TD><TD>"+dur+"</TD><TD>"+row[7][2:]+"</TD><TD>"+row[6]+"</TD><TD>"+row[4]+"</TD></TR>"
                                   my_list.append(row[10])
                                   n += 1
                               else:
                                   hline="<TR style=\"background-color:#262B31; height:30px; color:#fff;\"><TD>"+row[0][:10]+"</TD><TD>"+row[0][11:16]+"</TD><TD><font color=#0066ff><b><a target=\"_blank\" href=https://qrz.com/db/"+row[11]+">"+row[11]+"</a></b></font><span style=\"font: 7pt arial,sans-serif\"> ("+row[10]+")</span></TD><TD><font color=#ff0><b>"+row[12]+"</b></font></TD><TD><font color=#b5651d><b>"+row[8][2:]+"</b></font></TD><TD><font color=green><b>"+row[9]+"</b></font></TD><TD>"+dur+"</TD><TD>"+row[7][2:]+"</TD><TD>"+row[6]+"</TD><TD>"+row[4]+"</TD></TR>"
                                   my_list.append(row[10])
                                   n += 1
                               f.write(hline+"\n")
                            if n == 10:
